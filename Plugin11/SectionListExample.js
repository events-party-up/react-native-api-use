...
const renderSectionFooter = ({section}) => {
  console.log('renderSectionFooter ：', section)
  return (
    <View>
      <Text style={styles.headerText}>区块底部 SECTION FOOTER: {section.key}</Text>
      <SeparatorComponent />
    </View>
  );
}
...
export default class SectionListExample extends React.PureComponent {
  ...
  // 'viewOffset'（数字） - 用于抵消最终目标位置的固定数量的像素，例如用于补偿粘性标题。
  // 'viewPosition'（数字） - 将0索引指定的项目放置在顶部，1底部和0.5居中位置的值。
  scrollToLocation = (e, v) => {
    console.log('  onEndReached ：', e, v,  )
    this.section.scrollToLocation({animated: false, itemIndex: 2, sectionIndex: 3, viewOffset: 10, viewPosition: 1, })
    // this.section.scrollToLocation({animated: true, itemIndex: 2, sectionIndex: 3, viewOffset: 20, viewPosition: 1, })
  }
  ...
  flashScrollIndicators = (e, v) => {
    ...
    this.section.flashScrollIndicators()
  }
